...
from rilast.common.vae_training_utils import (
    calculate_latent_space,
    plot_latent_space_given,
    measure_rec_orig_similarity,
    verify_latent_statistics,
)
...
def sweep_train(config=None):
    ...
    with wandb.init(entity="dmhum", config=config):
        ...
        models = {
            "VecVAESimple": VecVAESimple(nDim, nLat),
            "VecVAE": VecVAE(nDim, nLat),
            "DeepVecVAE": DeepVecVAE(nDim, nLat),
            "DeepVecVAE2": DeepVecVAE2(nDim, nLat),
        }
        ...
        result_folder = (
            config["name"] + config.model + "_" + config.loss_func + "_train"
        )
        ...
        for epoch in range(epochs):
            ...
            wandb.log(
                {"train_loss": train_loss, "validation_loss": test_loss, "epoch": epoch}
            )
...
def cross_validation_sweep(config=None):
    ...
    for fold, (ix_train, ix_val) in enumerate(kf.split(archive)):
        ...
        run = wandb.init(
            entity="dmhum",
            project="rilast_cv_vehicle_2",
            config=config,
            group="experiment_01-04-2024",
            job_type="train",
            reinit=True,
            name=f"fold_{fold}",
        )
        ...
        train_load = DataLoader(
            dataset_train_, batch_size=batch_size, shuffle=True, num_workers=4
        )
        test_load = DataLoader(
            dataset_test_, batch_size=batch_size, shuffle=False, num_workers=4
        )
        ...
        for epoch in range(epochs):
            ...
            test_loss = test_step(
                model=model, test_loader=test_load, loss_function=loss_fn
            )

            mean_l, std_l, all_latents = verify_latent_statistics(model, test_load)
            similarity = measure_rec_orig_similarity(
                model, device, mean, std, dataset_test, generator
            )
...
if __name__ == "__main__":
    ...
    # archive = np.load("dataset_07_11_23_10k_17kappa.npy") # last used dataset
    # archive = np.load("datasets\\dataset_ads_16_10_24_10k.npy")
    # archive = np.load("datasets\\dataset_ads_22_10_24_10k_nov.npy")
    # archive = np.load("datasets\\dataset_ads_23_10_24_10k_nov_test_search_cos.npy")
    # archive = np.load("datasets\\22-10-24-art_ads.npy")

    # archive = np.load("datasets\\12-11-24-lhs_ads.npy")
    archive = np.load("datasets\\dataset_ads_17_11_24_10k_cos_nov.npy")

    # archive = np.load("dataset_13_11_23_10k_20kappa.npy")

    # archive = np.load("datasets\\01-02-2024-random_17_10k.npy")
    # archive = np.load("datasets\\14-01-2024-random_25_uav_10k.npy")
    ...
    sweep_config = {
        "method": "grid",
        "name": "17-11-24-novelty-lhs-ads_tanh",  #'29-01-2024-non-random-ads_tanh_-1_1',
        "metric": {"goal": "minimize", "name": "validation_loss"},
        "parameters": {
            "model": {
                "values": ["DeepVecVAE"]
            },  # "VecVAESimple", "VecVAE",   "DeepVecVAE2"
            "loss_func": {"values": ["lossA"]},  # , "lossB"
            "name": {
                "values": ["17-11-24-novelty-lhs-ads_tanh"]
            },  # 29-01-2024-non-random-ads_tanh_-1_1_deep
        },
    }
    ...
    sweep_id = wandb.sweep(
        sweep_config, project="17 -11-24-novelty-lhs-ads_tanh", entity="dmhum"
    )  # rilast_29-01-2024-non-random-ads_tanh_-1_1_deep
